...
begin
    if new.raw_app_meta_data is not null then
        ...
        if new.raw_app_meta_data ? 'provider' AND new.raw_app_meta_data ->> 'provider' = 'kakao' then
            ...
            values (new.id, new.raw_user_meta_data ->> 'name', 'kakao.' || substr(md5(random()::text), 1, 5), 'developer', new.raw_user_meta_data ->> 'avatar_url');
        end if;
        if new.raw_app_meta_data ? 'provider' AND new.raw_app_meta_data ->> 'provider' = 'github' then
            insert into public.profiles (profile_id, name, username, role, avatar) 
            values (new.id, new.raw_user_meta_data ->> 'full_name', 'github.' || substr(md5(random()::text), 1, 5), 'developer', new.raw_user_meta_data ->> 'avatar_url');
        end if;
    end if;
    return new;
end;
